...
def home():
    if request.method == 'POST':
        ...
        if dec.text == "poem":
            if them.text == "tragic":
                with open("/Service_1/doc/poem/sad/tragic") as trag:
                    tragic = trag.read()
                    story.append(tragic)
                    mit = story[0]
            if them.text == "sorrow":
                with open("/Service_1/doc/poem/sad/sorrow") as sorrow:
                    sor = sorrow.read()
                    story.append(sor)
                    mit = story[0]
            if them.text == "dark humour":
                with open("/Service_1/doc/poem/comedy/dark") as dark:
                    dark_humour = dark.read()
                    story.append(dark_humour)
                    mit = story[0]
            if them.text == "light humour":
                with open("/Service_1/doc/poem/comedy/light") as light:
                    light_humour = light.read()
                    story.append(light_humour)
                    mit = story[0]
            if them.text == "fluff":
                with open("/Service_1/doc/poem/romance/fluff") as fluff:
                    flu = fluff.read()
                    story.append(flu)
                    mit = story[0]
            if them.text == "intense":
                with open("/Service_1/doc/poem/romance/intense") as intense:
                    inten = intense.read()
                    story.append(inten)
                    mit = story[0]
        else:
            if them.text == "fluff":
                with open("/Service_1/doc/story/romance/fluff") as fluff:
                    flu = fluff.read()
                    story.append(flu)
                    lit = story[0].replace("___", characters.text.split(' ')[1])
                    mit = lit.replace("///", characters.text.split(' ')[0])
            if them.text == "intense":
                with open("/Service_1/doc/story/romance/intense") as inten:
                    intense = inten.read()
